        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
                {results && results.candidates && (
                    <>
                        <h2 className="text-xl font-bold text-blue-900 border-b pb-2">Propuestas de Sustitucion</h2>
                        {results.candidates.map((cand: any, idx: number) => (
                            <Card key={cand.id} className="border-l-4 border-l-green-500 shadow-md">
                                <div className="flex justify-between items-center mb-3">
                                    <h3 className="font-bold text-lg text-gray-800">Opcion {String.fromCharCode(65 + idx)}: {cand.name}</h3>
...
export default function IncidencePage() {
    return (
        <main className="min-h-screen p-8 max-w-7xl mx-auto font-[family-name:var(--font-geist-sans)]">
            ...
            <Suspense fallback={<div className="text-gray-500 text-center py-12">Cargando...</div>}>
                <IncidenceForm />
            </Suspense>
        </main>
    );
